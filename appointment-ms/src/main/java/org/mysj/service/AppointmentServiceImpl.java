...
class AppointmentServiceImpl implements AppointmentService {
    ...
    public void createAppointment(AppointmentDto appointment) {
        var patient = patientRestClient.get().uri("/patient", uriBuilder -> uriBuilder.queryParam("patientId", appointment.getPatientId()).build()).retrieve().onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
